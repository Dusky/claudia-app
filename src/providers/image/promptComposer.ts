...
export interface ImagePromptComponents {
  // Detailed, structured components for building a high-quality prompt
  subjectDescription: string;         // Detailed "Subject: Claudia..."
  poseAndExpression: string;        // "Pose / Expression: ..."
  settingDescription: string;       // "Setting: ..."
  atmosphereAndStyle: string;       // "Atmosphere / Style: ..."
  lightingDescription: string;      // "Lighting: ..."
  cameraPerspectiveAndComposition: string; // "Camera Perspective: ..."
  realismAndDetails: string;        // "Details & Realism Cues: ..."
  
  // Core keywords for general style and quality
  styleKeywords: string;            // Broader style terms like "digital art, photorealistic"
  qualityKeywords: string;          // Standard quality terms like "high quality, masterpiece"
  negativePrompt: string;

  // For AI-driven or meta-prompted scenarios
  // This will hold the primary description if provided by AI or generated by meta-LLM
  primaryDescription?: string; 

  // Still useful for variations or simpler state-to-prompt fallbacks if not using full detail
  baseCharacterReference: string; // Short "Claudia, ..." for appending
...
export class ImagePromptComposer {
  ...
  > = {
    ...
    baseCharacterReference: "Claudia" // Short reference for appending
  };
  ...
  generatePromptComponents(params: ExtendedAvatarGenerationParams): ImagePromptComponents {
    const components = { ...this.basePrompts } as ImagePromptComponents; // Start with base

    // Store primary description if provided by meta-LLM or AI
    ...
    // Populate detailed fields based on current state, these will be used if no primaryDescription
    // or can be used by meta-LLM as input.
    ...
    // Allow specific style, lighting, background from params to override defaults
    ...
    // Populate simpler keywords for fallback or meta-LLM input
    components.expressionKeywords = this.expressionKeywordsMap[params.expression || 'neutral'];
    ...
    // Apply variations if a seed is provided and not primarily driven by AI/meta-prompt
    ...
    // Apply contextual keywords if not primarily driven by AI/meta-prompt
  ...
  async applyPersonalityModifications(
    ...
    if (systemPrompt.includes('technical expert') || systemPrompt.includes('highly analytical')) {
      ...
      if (!isPrimaryDescriptionDriven) { // Only override if not AI/meta driven
    ...
    // Ensure core character identity is part of subjectDescription if a primary description is driving things
    if (isPrimaryDescriptionDriven) {
  ...
  private ensureBaseCharacter(description: string): string {
      ...
      // Check if core traits are missing
  ...
  compilePrompt(components: ImagePromptComponents): string {
    if (components.primaryDescription && components.primaryDescription.trim().length > 0) {
      // If a primary description (from meta-LLM or AI [IMAGE:] tag) exists, use it as the main body.
      // Append essential identifiers and quality.
      let finalPrompt = components.primaryDescription;
      // Ensure Claudia's name and core traits are mentioned if not already in the primary description.
  ...
  getNegativePrompt(components: ImagePromptComponents): string {
    // Start with the base negative prompt
    let finalNegativePrompt = components.negativePrompt || this.basePrompts.negativePrompt || '';
    
    // If there's a primary description (from AI/Meta-LLM), check for implied negatives
